...
//	public int tileNXY[][] = new int [worldW][worldH];
	...
	public Block[][] block = new Block [CHUNK_SIZE * NUMBER_OF_CHUNKS_ON_ONE_SIDE][CHUNK_SIZE * NUMBER_OF_CHUNKS_ON_ONE_SIDE];
 ...
 */
	...
	public void building(int camX, int camY, int renW, int renH) {
		if(Component.isMouseLeft || Component.isMouseRight) {
			for(int x=(camX /TILE_SIZE)-(chunkOffsetX*CHUNK_SIZE); x<(camX / TILE_SIZE) + renW-(chunkOffsetX*CHUNK_SIZE);x++) {
				for(int y=(camY / TILE_SIZE)-(chunkOffsetY*CHUNK_SIZE); y<(camY / TILE_SIZE) + renH-(chunkOffsetY*CHUNK_SIZE);y++) {
					if(x >= 0 && y >= 0 && x < worldW && y < worldH) {
						if(block[x][y].contains(new Point((Component.mse.x / Component.pixelSize) + (int) Component.sX, (Component.mse.y / Component.pixelSize) + (int) Component.sY))) {
							...
							int chunkGridX = x/CHUNK_SIZE; 
							int chunkGridY = y/CHUNK_SIZE;
							chunk[chunkGridX][chunkGridY].cBlock[x-(chunkGridX*CHUNK_SIZE)][y-(chunkGridY*CHUNK_SIZE)]= block[x][y];
...
//			Component.mobID = mobID2+1;
		...
		if(Component.character.x < block[0][0].x + CHUNK_SIZE *TILE_SIZE - TILE_SIZE/2) {
			File f = new File("chunks/0-0.sav");
			//if(Component.character.x> CHUNK_SIZE*TILE_SIZE) {
		...
		if(Component.character.y < block[0][0].y + CHUNK_SIZE *TILE_SIZE - TILE_SIZE) {
		...
		if(Component.character.x > block[0][0].x + CHUNK_SIZE * 2 *TILE_SIZE) {
		...
		if(Component.character.y > block[0][0].y + CHUNK_SIZE * 2 *TILE_SIZE - TILE_SIZE) {
		...
		for(int x=(camX /TILE_SIZE)-(chunkOffsetX*CHUNK_SIZE); x<(camX / TILE_SIZE) + renW-(chunkOffsetX*CHUNK_SIZE);x++) {
			for(int y=(camY / TILE_SIZE)-(chunkOffsetY*CHUNK_SIZE); y<(camY / TILE_SIZE) + renH-(chunkOffsetY*CHUNK_SIZE);y++) {
				if(x >= 0 && y >= 0 && x < worldW && y < worldH) {
					...
					//block2[x][y].render(g);
					//block3[x][y].render(g);
					if(block[x][y].id != AIR && block[x][y].id != AIR && !Inventory.isOpen) {
						if(block[x][y].contains(new Point((Component.mse.x / Component.pixelSize) + (int) Component.sX, (Component.mse.y / Component.pixelSize) + (int) Component.sY))) {
							g.setColor(new Color(255,255,255, 40));
							g.drawRect(block[x][y].x - camX, block[x][y].y - camY, block[x][y].width-1, block[x][y].height-1);
						}
...
//		g.drawRect (camX-(chunkOffsetX*CHUNK_SIZE),camY-(chunkOffsetY*CHUNK_SIZE*TILE_SIZE),renW*TILE_SIZE,renH*TILE_SIZE);
//		
//		for(int x=(Component.cameraX/TILE_SIZE)-(chunkOffsetX* CHUNK_SIZE); x<(Component.cameraX / TILE_SIZE) + (Component.cameraW/TILE_SIZE)-(chunkOffsetX*CHUNK_SIZE);x++) {
//			for(int y=(Component.cameraY / TILE_SIZE)-(chunkOffsetY* CHUNK_SIZE); y<(Component.cameraY / TILE_SIZE) + (Component.cameraH/TILE_SIZE)-(chunkOffsetY* CHUNK_SIZE);y++) {
//				if(x >= 0 && y >= 0 && x < worldW && y < worldH) {
//					block[x][y].render(g);
//					if(block[x][y].id != Tile.air && block[x][y].id != Tile.bedrock && !Inventory.isOpen) {
//						if(block[x][y].contains(new Point((Component.mse.x / Component.pixelSize) + (int) Component.sX, (Component.mse.y / Component.pixelSize) + (int) Component.sY))) {
